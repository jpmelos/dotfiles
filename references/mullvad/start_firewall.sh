...
sudo iptables -A INPUT -i default -m conntrack --ctstate ESTABLISHED,RELATED -j ACCEPT
sudo iptables -A INPUT -p udp -m udp -i wlp2s0 -s 185.65.135.223 --sport 51820 -m conntrack --ctstate ESTABLISHED,RELATED -j ACCEPT

sudo iptables -A OUTPUT -o default -m conntrack --ctstate NEW,ESTABLISHED -j ACCEPT
